/*
 Name:		OBD_CAN_SIMULATOR.ino
 Created:	12/2/2020 10:17:47 AM
 Author:	bvanpelt
*/
 ...
 *  Modification and updates by: Brandon Van Pelt   December 1st, 2020
 */
...
#define LISTEN_ID 0x7E8
#define LISTEN_ID2 0x7E0
#define FUNCTIONAL_ID 0x7E8  
...
void entry()
{
    if (Can0.get_rx_buff(incCAN0))                         // If CAN0_INT pin is low, read receive buffer
    {
        rxId = incCAN0.id;
        dlc = incCAN0.length;
        rxBuf[0] = incCAN0.data.bytes[0];
        rxBuf[1] = incCAN0.data.bytes[1];
        rxBuf[2] = incCAN0.data.bytes[2];
        rxBuf[3] = incCAN0.data.bytes[3];
        rxBuf[4] = incCAN0.data.bytes[4];
        rxBuf[5] = incCAN0.data.bytes[5];
        rxBuf[6] = incCAN0.data.bytes[6];
        rxBuf[7] = incCAN0.data.bytes[7];

        Serial.print("ID ");// Get CAN data
        Serial.println(uint16_t(value), HEX);
        Serial.print("MODE ");// Get CAN data
        Serial.println(uint8_t(rxBuf[1]), HEX);
        Serial.print("PID ");// Get CAN data
        Serial.println(uint8_t(rxBuf[2]), HEX);
        // First request from most adapters...
        uint16_t test = rxId;
        if ((test == 0x7DF) || (test = 0x7E0)) {
            obdReq(rxBuf);
        }
    }
}
...
void obdReq(byte* data) {
    ...
    //=============================================================================
        ...
        //    }
            ...
            //byte VIN[] = { (0x40 | mode), pid, 0x01, 0x31, 0x47, 0x31, 0x59, 0x59, 0x32, 0x36, 0x55, 0x58, 0x36, 0x35, 0x31, 0x32, 0x33, 0x34, 0x36, 0x39 };
            //2006 corv 1G1YY26UX65123469              31    47    31    59    59    32    36    55    58    36    35    31    32    33    34    36    39
  ...
  //  }
    ...
    if (tx)
    {
        ...
        CANOut.id = REPLY_ID;
        CANOut.length = 8;
        CANOut.data.bytes[0] = txData[0];
        CANOut.data.bytes[1] = txData[1];
        CANOut.data.bytes[2] = txData[2];
        CANOut.data.bytes[3] = txData[3];
        CANOut.data.bytes[4] = txData[4];
        CANOut.data.bytes[5] = txData[5];
        CANOut.data.bytes[6] = txData[6];
        CANOut.data.bytes[7] = txData[7];
...
void iso_tp(byte mode, byte pid, int len, byte* data) {
    ...
    CANOut.id = REPLY_ID;
    CANOut.length = 8;
    CANOut.data.bytes[0] = tpData[0];
    CANOut.data.bytes[1] = tpData[1];
    CANOut.data.bytes[2] = tpData[2];
    CANOut.data.bytes[3] = tpData[3];
    CANOut.data.bytes[4] = tpData[4];
    CANOut.data.bytes[5] = tpData[5];
    CANOut.data.bytes[6] = tpData[6];
    CANOut.data.bytes[7] = tpData[7];
    Can0.sendFrame(CANOut);
    ...
    while (not_done) {
        ...
        if (Can0.get_rx_buff(incCAN0)) {
            ...
            if (((uint16_t(rxId) == 0x7DF) || (uint16_t(rxId) == 0x7E0)) && ((rxBuf[0] & 0xF0) == 0x30)) {
